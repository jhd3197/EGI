...
import { HomeIcon, SearchIcon, SheltersIcon, MineIcon, MeshIcon, MapIcon, ChartIcon, RouteIcon, SettingsIcon, OperationsIcon, PawIcon } from './Icons.jsx'
...
export default function Sidebar({ view, actions }) {
  ...
  return (
    <div style={{ ...css('flex-direction:column;width:256px;flex:none;height:100%;background:#fff;border-right:1px solid #ECE8E2;padding:22px 16px;gap:5px;'), display: v.sidebarDisplay }}>
      ...
      {v.showAnimalsTab && (
        <NavButton onClick={() => actions.setScreen('animals')} nav={v.navAnimals} icon={<PawIcon />} label={t('nav.animals')} />
      )}
      {v.showOperationsTab && (
